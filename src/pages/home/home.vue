    ...
    <div v-show="!showLoading">
      ...
      <div class="recommend-box">
        <div v-for="item in recommendData"
          ...
          <p v-else class="tip-text">暂无 * _ *</p>
        </div>
      </div>
      <div class="load-more"
           @click="loaderMore">
...
export default {
  ...
  methods: {
    ...
    async getRecommendApi () {
      let data = await getRecommend(this.next_date).then(res => res.json())
      this.next_date = data.date
      this.recommendData.push({ time: this.next_date, data: data.recommend_feeds })
    },
    // 获取话题
    async getQuickDataApi () {
      let data = await getQuickData().then(res => res.json())
      let i = Math.floor(Math.random() * 10)
      this.quickData = data[i]
    }
  }
